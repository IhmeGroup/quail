...
def test_case():
    ...
    with open('Data_final.pkl', 'rb') as f:
        ...
        face0 = solver.mesh.elements[2].faces[1].children[0]
        face1 = solver.mesh.elements[2].faces[1].children[1]
        # Verify neighbors of new faces
        np.testing.assert_equal(face0.elemL_ID, 2)
        np.testing.assert_equal(face0.elemR_ID, 1)
        np.testing.assert_equal(face1.elemL_ID, 2)
        np.testing.assert_equal(face1.elemR_ID, 4)
        # Verify face IDs of new faces
        np.testing.assert_equal(face0.faceL_ID, 1)
        np.testing.assert_equal(face0.faceR_ID, 1)
        np.testing.assert_equal(face1.faceL_ID, 1)
        np.testing.assert_equal(face1.faceR_ID, 1)
