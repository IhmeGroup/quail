...
class ADERDG(DG):
	...
	def calculate_residual_iface(self, iiface, UpL, UpR, RL, RR):
		...
		RL -= np.matmul(np.tile(basis_valL,(nb,1)).transpose(), Fq*quad_wts_st) # [nb, ns]
		RR += np.matmul(np.tile(basis_valR,(nb,1)).transpose(), Fq*quad_wts_st) # [nb, ns]
	...
	def calculate_residual_bface(self, ibfgrp, ibface, U, R):
		...
		R -= np.matmul(np.tile(basis_val,(nb,1)).transpose(),Fq*quad_wts_st)
