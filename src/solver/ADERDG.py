...
class ADERDG(base.SolverBase):
	...
	def get_element_residual(self, Uc, res_elem):
		...
		if fluxes:
			...
			Fq = physics.get_conv_flux_interior(Uq, x=None, t=None)[0] # [ne, nq, ns, ndims]
	...
	def get_interior_face_residual(self, faceL_IDs, faceR_IDs, UcL, UcR):
		...
		if fluxes:
			...
			Fq = physics.get_conv_flux_numerical(UqL, UqR, normals_int_faces, x=None, t=None)
					# [nf, nq_st, ns]
	...
	def flux_coefficients(self, dt, order, basis, Up):
		...
		if InterpolateFluxADER:
			# Calculate flux
			Fq = physics.get_conv_flux_interior(Up, x=None, t=None)[0]
			...
			Fq = physics.get_conv_flux_interior(Uq, x=None, t=None)[0]
