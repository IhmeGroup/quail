...
class EulerMultispecies1D_11sp_H2O2(EulerMultispecies1D):
	'''
	This class corresponds to 1D Euler equations with simple chemistry.
	It inherits attributes and methods from the Chemistry class.
	See Chemistry for detailed comments of attributes and methods.
	...
	NUM_STATE_VARS = 13
	NUM_SPECIES  = 11
	PHYSICS_TYPE = general.PhysicsType.EulerMultispecies1D_11sp_H2O2
	CANTERA_FILENAME = "h2o2_11sp_23rxn.xml"
	...
	def compute_additional_variable(self, var_name, Uq, flag_non_physical):
		...
		if vname is self.AdditionalVariables["MassFractionH2"].name:
			...
			varq = rhoYH2O2/rho		
		elif vname is self.AdditionalVariables["MassFractionN"].name:
			varq = rhoYN/rho		
		elif vname is self.AdditionalVariables["MassFractionNO"].name:
			varq = rhoYNO/rho		
		else:
			varq = super().compute_additional_variable(var_name, Uq, 
					flag_non_physical)
